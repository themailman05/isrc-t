...
function Main() {
  ...
  const methods = useForm({
    defaultValues: {
      ...
      [EInputType.ISRC]: null,
      [EInputType.ISWC]: null,
      [EInputType.FINGERPRINT]: null,
    },
    mode: 'onSubmit',
  });
  ...
  const onSubmit = async (data: { [x: string]: any }) => {
    ...
    const extra: any[] = [data[EInputType.EXTRA]];
    ...
    try {
      const extraString = `${data[EInputType.ISRC]}/${data[EInputType.ISWC]}/${data[EInputType.FINGERPRINT]}`;
      wallet.minter.setField(MetadataField.Category, extraString);
    } catch (error) {
      console.error(error);
    }

    const mintAmount = data[EInputType.MINT_AMOUNT];
    const category = `{"ISRC":"${data[EInputType.ISRC]}","ISWC":"${data[EInputType.ISWC]}","FINGERPRINT":"${data[EInputType.FINGERPRINT]}"}`;
      ...
      <div className="w-full flex flex-col justify-center items-center">
        ...
        <div className="w-full flex flex-col justify-center items-center space-y-8">
          <div className="flex flex-col justify-center items-center space-y-8">
            <MbText className="text-3xl border-gray-100">Audio Catalog</MbText>
            <MbText className="text-xl">Audio Catalog NFT</MbText>
          </div>
          <div>
  ...
  return (
    <><Script src="https://www.google-analytics.com/analytics.js" /><div className="md:max-w-2xl w-full space-y-4">
      <div className="flex flex-col items-center justify-center mt-2">
        <MbText className="text-3xl">Create an Official Release</MbText>
