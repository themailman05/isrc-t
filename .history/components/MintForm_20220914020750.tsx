...
function MintForm() {
  ...
  const uploadSound = (file:File) => {
    setValue(EInputType.FINGERPRINT, chromaprint.calculateFingerprint(file.stream, console.log));
    setValue(EInputType.FOREVER_MEDIA, file);
    clearErrors(EInputType.MAIN_IMAGE);
  };

  const Chromaprint = dynamic(() => import("chromaprint").then((mod) => mod.Widget),
    ssr: false
  )
  ...
  return (
    <>
      ...
      <div className="mb-4">
        <MbNearAmountAccount
          accountExists={function noRefCheck(name: String){
